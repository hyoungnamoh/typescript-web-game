...
var horizontal = 4;
var vertical = 3;
var colors = ['red', 'orange', 'green', 'yellow', 'white', 'red', 'orange', 'green', 'yellow', 'white', 'pink', 'pink'];
...
var shuffle = function () {
    for (var i = 0; 0 < colorCandidate.length; i++) { //colorCandidate 자체 길이가 한 번 돌때마다 줄어들기 때문에 i로 for문을 제어할 필요가 없음
        color = color.concat(colorCandidate.splice(Math.floor(Math.random() * colorCandidate.length), 1));
    }
};
var setCard = function (horizontal, vertical) {
    clickFlag = false;
